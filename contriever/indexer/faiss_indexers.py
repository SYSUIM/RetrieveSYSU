...
class DenseIndexer(object):
    ...
    def serialize(self, file: str):
        print("Serializing index to %s" % file)
    ...
    def deserialize(self, path: str):
        print("Loading index from %s" % path)
        index_file, meta_file = self.get_files(path)
...
class DenseFlatIndexer(DenseIndexer):
    ...
    def index_data(self, data: List[Tuple[object, np.array]]):
        ...
        for i in range(0, n, self.buffer_size):
            ...
            print("data indexed %d", total_data)

        indexed_cnt = len(self.index_id_to_db_id)
        print("Total data indexed %d" % indexed_cnt)
...
class DenseHNSWFlatIndexer(DenseIndexer):
    ...
    def index_data(self, data: List[Tuple[object, np.array]]):
        ...
        for i, item in enumerate(data):
            ...
            phi = max(phi, norms)
        print("HNSWF DotProduct -> L2 space phi={}".format(phi))
        self.phi = phi
        ...
        for i in range(0, n, bs):
            ...
            print("data indexed %d" % len(self.index_id_to_db_id))
        indexed_cnt = len(self.index_id_to_db_id)
        print("Total data indexed %d" % indexed_cnt)
    ...
    def search_knn(self, query_vectors: np.array, top_docs: int) -> List[Tuple[List[object], List[float]]]:
        ...
        print("query_hnsw_vectors %s" % query_nhsw_vectors.shape)
